...
This directory contains automation scripts for RPG program analysis and migration. All scripts support both Unix/Linux/macOS and Windows platforms.

## Available Scripts

### 1. analyze-dependencies (.sh / .ps1)
Scans RPG source files and generates a dependency graph in JSON format.
...
**Output:** `dependencies.json` containing programs, dependencies, service programs, and file references.

---

### 2. extract-structure.py
Extracts structural information from RPG source files including specifications, variables, subroutines, and dependencies.
...
```bash
# Output to stdout
python3 extract-structure.py program.rpgle

# Save to file
python3 extract-structure.py program.rpgle -o structure.json
```
...
```bash
# Print to stdout
python3 generate-java-classes.py CUSTOMER.dds

# Generate to specific package
python3 generate-java-classes.py CUSTOMER.dds -p com.myapp.model -o ./src/main/java
```
...
**Usage:**
```bash
python3 estimate-complexity.py <rpg_file> [--output report.json]
```
