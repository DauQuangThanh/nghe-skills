...
This directory contains automation scripts for JCL job analysis and migration. All scripts support both Unix/Linux/macOS and Windows platforms.

## Available Scripts

### 1. analyze-dependencies (.sh / .ps1)
Scans JCL job files and generates a dependency graph in JSON format.
...
**Output:** `dependencies.json` containing jobs, job steps, program dependencies, and dataset references.

---

### 2. extract-structure.py
Extracts structural information from JCL files including job steps, programs, datasets, and conditional logic.
...
```bash
# Output to stdout
python3 extract-structure.py JOBCARD.jcl

# Save to file
python3 extract-structure.py JOBCARD.jcl -o structure.json
```

---

### 3. generate-java-classes.py
Generates Java POJO classes from legacy data structures.

**Usage:**
```bash
python3 generate-java-classes.py <data_structure_file> [options]
```
...
```bash
# Print to stdout
python3 generate-java-classes.py DATASET.copy

# Generate to specific package
python3 generate-java-classes.py DATASET.copy -p com.myapp.model -o ./src/main/java
```
...
**Usage:**
```bash
python3 estimate-complexity.py <jcl_file> [--output report.json]
```

**Factors considered:**
- Number of job steps
- Conditional execution (COND parameters)
- Program calls
- Dataset operations
- Utility programs (SORT, IEBGENER, etc.)
- Job dependencies
