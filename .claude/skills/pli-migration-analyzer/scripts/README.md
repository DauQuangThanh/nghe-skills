...
This directory contains automation scripts for PL/I program analysis and migration. All scripts support both Unix/Linux/macOS and Windows platforms.

## Available Scripts

### 1. analyze-dependencies (.sh / .ps1)
Scans PL/I source files and generates a dependency graph in JSON format.
...
**Output:** `dependencies.json` containing programs, dependencies, include files, and external procedures.

---

### 2. extract-structure.py
Extracts structural information from PL/I source files including procedures, variables, and dependencies.
...
```bash
# Output to stdout
python3 extract-structure.py program.pli

# Save to file
python3 extract-structure.py program.pli -o structure.json
```
...
```bash
# Print to stdout
python3 generate-java-classes.py CUSTOMER.inc

# Generate to specific package
python3 generate-java-classes.py CUSTOMER.inc -p com.myapp.model -o ./src/main/java
```
...
**Usage:**
```bash
python3 estimate-complexity.py <pli_file> [--output report.json]
```
